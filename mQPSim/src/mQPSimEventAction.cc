...
void mQPSimEventAction::EndOfEventAction(const G4Event* event)
{
 ...
 if ( fPMTgammaHCID == -1 ) {
   fPMTgammaHCID
     = G4SDManager::GetSDMpointer()->GetCollectionID("PMTMFD/nGamma");
 }

 // Get sum values from hits collections
 //
 auto PMTgamma = GetSum(GetHitsCollection(fPMTgammaHCID, event));
  ...
  analysisManager->FillH1(4, PMTgamma);
//  analysisManager->FillH1(3, fEdep);
  ...
  //  G4cout << "---> End of event output: " << eventID << G4endl;
    //G4cout
       ...
       //<< "       Number of photons hitting the PMTface: "
       //<< std::setw(7) << PMTgamma
      // << G4endl;
  //}
}
